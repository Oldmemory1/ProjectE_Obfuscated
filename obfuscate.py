...
RESOURCES = os.path.join(OBF_DIR, "main", "resources")

all_java = []
for d in [MAIN_JAVA, API_JAVA, TEST_JAVA]:
    if os.path.exists(d):
...
with open(BUILD_GRADLE, 'r', encoding='utf-8') as f:
    bg = f.read()
bg_count = 0
for old, new in [(OLD_PKG, NEW_PKG), (OLD_PKG_PATH, NEW_PKG_PATH), (OLD_MAVEN, NEW_MAVEN)]:
    c = bg.count(old)
    if c > 0:
        bg = bg.replace(old, new)
        bg_count += c
with open(OBF_BUILD_GRADLE, 'w', encoding='utf-8') as f:
    f.write(bg)
